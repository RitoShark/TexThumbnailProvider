             ...
             header.tex_format == tex_format_bc7  || header.tex_format == tex_format_bc5)
    {
        ...
        if (dstW == width && dstH == height)
        {
            ...
            case tex_format_dxt5:
                ...
                break;
            case tex_format_bc7:
                BlockDecompressImageBC7(width, height, dxtData.get(), pixels);
                break;
            case tex_format_bc5:
                BlockDecompressImageBC5(width, height, dxtData.get(), pixels);
                break;
        ...
        else if (header.tex_format == tex_format_dxt1 || header.tex_format == tex_format_dxt5)
        {
            BoxDownsampler ds(width, height, dstW, dstH);
        ...
        else // BC7 and BC5 dont have single block decoder, decodes full and downsamples
        {
            malloc_ptr<unsigned long> imageData((unsigned long*) malloc((size_t)stride * height));
            if (!imageData)
            {
                DeleteObject(hBitmap);
                return E_OUTOFMEMORY;
            ...
            if (header.tex_format == tex_format_bc7)
                BlockDecompressImageBC7(width, height, dxtData.get(), imageData.get());
            else
                BlockDecompressImageBC5(width, height, dxtData.get(), imageData.get());

            BoxDownsampler ds(width, height, dstW, dstH);
            const unsigned long* px = imageData.get();
            for (UINT y = 0; y < height; ++y)
            {
                for (UINT x = 0; x < width; ++x)
                {
                    unsigned long rgba = px[(size_t)y * width + x];
                    ds.Add(x, y,
                           (uint8_t)((rgba >> 8) & 0xFF),
                           (uint8_t)((rgba >> 16) & 0xFF),
                           (uint8_t)((rgba >> 24) & 0xFF),
